/* =========================================================
   Sessão e proteção de rota (só ADMIN acessa esta tela)
   ========================================================= */
...
document.getElementById("nomeUsuarioLogado").textContent = sessao ? sessao.nome : "administrador";
...
function lerDataBR(valor) {
    ...
    return data.getFullYear() === partes[2] &&
        data.getMonth() === partes[1] - 1 &&
        data.getDate() === partes[0] &&
        data <= hoje ? data : null;
}
...
dataNascimento.addEventListener("input", function () {
    ...
    dataSelecionada = lerDataBR(this.value);
    this.setCustomValidity(
        digitos.length === 8 && !dataSelecionada ? "Informe uma data válida." : ""
    );
...
dataNascimento.addEventListener("keydown", function (evento) {
        ...
        usuarios.forEach(usuario => {
            ...
            tabela.innerHTML += `
                <tr>
                    ...
                    <td>
                        <div class="acoes">
                            <button class="pushable small editar icone" type="button"
                                onclick="editarUsuario(${usuario.id}, this)"
                                title="Editar" aria-label="Editar usuário">
                                <span class="shadow"></span>
                                <span class="edge"></span>
                                <span class="front">
                                    <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M12 20h9M16.5 3.5a2.121 2.121 0 013 3L7 19l-4 1 1-4 12.5-12.5z" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/>
                                    </svg>
                                </span>
                            </button>

                            <button class="pushable small excluir icone" type="button"
                                onclick="excluirUsuario(${usuario.id}, this)"
                                title="Excluir" aria-label="Excluir usuário">
                                <span class="shadow"></span>
                                <span class="edge"></span>
                                <span class="front">
                                    <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M4 7h16M9 7V4a1 1 0 011-1h4a1 1 0 011 1v3m2 0v13a2 2 0 01-2 2H8a2 2 0 01-2-2V7h12zM10 11v6M14 11v6" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/>
                                    </svg>
                                </span>
                            </button>
                        </div>
...
form.addEventListener("submit", async function (e) {
    ...
    if (criando && !senha.value.trim()) {
        mostrarAviso("Defina uma senha para o novo usuário.", "Dados inválidos");
        return;
    }
...
async function editarUsuario(codigo, botao) {
    ...
    try {
        ...
        cpf.value = formatarCPF(usuario.cpf);
        telefone.value = formatarTelefone(usuario.telefone);
